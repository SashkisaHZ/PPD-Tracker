@php
    ...
    $feedbacks = session("year2_feedback", []);
@endphp
        ...
        <div class="flex-grow space-y-6 overflow-y-auto pr-2">
            @foreach($elementsProgress as $index => $element)
                <div class="bg-gray-50 p-4 rounded-lg shadow-sm border border-gray-200">
                    ...
                    @if($isTeacher)
                        <form id="edit-form-{{ $index }}" method="POST" action="{{ route('tasks.update', [2, $index]) }}"
                        ...
                        <form id="feedback-form-{{ $index }}" method="POST" action="{{ route('tasks.feedback', [2, $index]) }}"
                              class="flex items-center space-x-2 mt-2 hidden">
                            @csrf
                    ...
                    @if(isset($feedbacks[$index]) && $feedbacks[$index])
                        ...
                        @if($isTeacher)
                            <form id="feedback-edit-form-{{ $index }}" method="POST" action="{{ route('tasks.feedback', [2, $index]) }}"
                                  class="flex items-center space-x-2 mt-2 hidden">
                                @csrf
                                ...
                                <button type="button" class="bg-gray-400 text-white px-3 py-1 rounded" onclick="hideFeedbackEdit({{ $index }})">Cancel</button>
                            </form>
                        @endif
                    @endif
                </div>
